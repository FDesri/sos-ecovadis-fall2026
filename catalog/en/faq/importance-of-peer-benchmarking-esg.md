---
id: kb-0107
type: faq
lang: en
source_lang: en
translation_of: null
title: "How important is peer benchmarking when developing an ESG strategy?"
slug: importance-of-peer-benchmarking-esg
summary: >-
  Benchmarking against peers and understanding customer expectations are
  crucial to stay competitive: knowing the ESG landscape of competitors
  enables better strategic alignment while initiatives stay tailored to your
  own maturity.
description: "Benchmarking against peers and understanding customer expectations is what aligns an ESG strategy with its market rather than with nothing."
situations: [S1, S2, S3]
sizes: [XS, S]
content_kind: general
intent: comprendre
themes: [general]
topics: [methodologie, medailles-percentiles]
ecovadis_questions: []
keywords: [peer benchmarking, ESG strategy, competitive position]
audience: [sustainability_lead, sme_owner]
author: francois-dequenne
expert_reviewer: francois-dequenne
source: esgim-proprietary
source_note: "FAQ 2025 — ESG webinars Q&A (Evernote, updated 2026-08-24)"
sources: [ecovadis-medals, ecovadis-methodology]
confidentiality: public
reliability: expert-drafted
date_created: 2026-08-27
date_updated: 2026-08-27
verified_at: 2026-08-27
volatility: ecovadis-cycle
review_due: 2027-02-27
version: 1
status: published
related: [ecovadis-rising-standards-2026, ecovadis-thresholds-belgian-packaging-pharma]
---

# How important is peer benchmarking when developing an ESG strategy?

Benchmarking against peers and understanding customer expectations are **crucial to staying competitive**. While companies rightly tailor initiatives to their own business model and maturity, knowing the ESG landscape of competitors enables better strategic alignment and addresses stakeholder demands more effectively. In the [EcoVadis](https://resources.ecovadis.com/whitepapers/ecovadis-ratings-methodology-overview-and-principles-2022-neutral) context this is doubly true: since medals are percentile-based, your position is literally defined relative to your peers.
